...
interface Props {
  ...
  /** Set when the cost center must not be written to — e.g. it is locked. */
      ...
      <div>
        ...
        <div className="p-5 space-y-5">
            ...
            <div className="border border-gray-200 rounded-lg divide-y divide-gray-100 max-h-52 overflow-y-auto">
              ...
              {groups.length === 0 && (
                <p className="px-3 py-3 text-xs text-gray-500">
                  Inga budgeterbara konton i det här kostnadsstället.
                </p>
              )}
            </div>
          </div>
          ...
          {done !== null && (
            <div className="flex items-start gap-2 p-3 bg-green-50 border border-green-200 rounded-lg text-xs text-green-800">
              ...
              <span>{fmt(done)} celler sparade. Öppna kostnadsstället i budgetvyn för att granska.</span>
            </div>
          )}
        ...
        <div className="flex justify-end p-4 border-t border-gray-200">
          <button
            onClick={handleApply}
            disabled={
              plan.cells.length === 0 || applying || futurePeriods.length === 0 || !!blockedReason
            }
